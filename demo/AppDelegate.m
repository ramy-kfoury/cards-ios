...
- (void) displayAlertView:(NSString *)message WithTitle:(NSString *)title AndCancelTitle:(NSString *)cancelTitle OtherButtonTitle:(NSString *)otherTitle fromDelegate:(id)delegate
{
    if (_alertView == nil)
    {
        _alertView = [[UIAlertView alloc] initWithTitle:title message:message delegate:delegate cancelButtonTitle:cancelTitle otherButtonTitles:otherTitle, nil];
        [_alertView show];
    }
    else
    {
        [_alertView setDelegate:delegate];
